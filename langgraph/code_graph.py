...
def conditional_query(state: State) -> Literal["__end__", "coding_query"]:
    try:
        accuracy = float(state["accuracy_percentage"].strip('%'))
    ...
    if accuracy >= 95 or state["retry_count"] > 5:
        return END
    else:
        return "coding_query"
...
graph_builder.add_node("conditional_query", conditional_query)
...
graph_builder.add_conditional_edges("coding_validate_query", conditional_query)
